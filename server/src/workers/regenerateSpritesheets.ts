...
async function main(cx: ServerContext) {
  ...
  try {
    ...
    cx.logger.log(`1st pass: ${malesWithSkin.length} possible visuals`);
    ...
    cx.logger.log(`2nd pass: ${malesWithHair.length} possible visuals`);
    ...
    cx.logger.log(`3rd pass: ${malesWithPrimary.length} possible visuals`);
    ...
    cx.logger.log(`4th pass: ${malesWithSecondary.length} possible visuals`);
    ...
    cx.logger.log(`5th pass: ${malesWithSecondary.length + femalesWithSkin.length} possible visuals`);
    ...
    cx.logger.log(`6th pass: ${malesWithSecondary.length + femalesWithHair.length} possible visuals`);
    ...
    cx.logger.log(`7th pass: ${malesWithSecondary.length + femalesWithPrimary.length} possible visuals`);
    ...
    cx.logger.log(`8th pass: ${allVisuals.length} possible visuals`);
    cx.logger.log('Regenerating all spritesheets...');

    for (let i = 0; i < allVisuals.length; i++) {
      const visuals = allVisuals[i];

      // Check if spritesheet already exists and is up to date
      const existingSpritesheet = await cx.prisma.bruteSpritesheet.findUnique({
        where: {
      ...
      if (existingSpritesheet && existingSpritesheet.version === SPRITESHEET_VERSION) {
        cx.logger.log(`Skipped spritesheet for visual ${i + 1}/${allVisuals.length}`);
      ...
      await cx.prisma.bruteSpritesheet.upsert({
        where: {
          // eslint-disable-next-line max-len
          gender_longHair_lowerRightArm_rightHand_upperRightArm_rightShoulder_rightFoot_lowerRightLeg_upperRightLeg_leftFoot_lowerLeftLeg_pelvis_upperLeftLeg_tummy_torso_head_leftHand_upperLeftArm_lowerLeftArm_leftShoulder_skinColor_skinShade_hairColor_hairShade_primaryColor_primaryShade_secondaryColor_secondaryShade_accentColor_accentShade: visuals,
        },
      ...
      cx.logger.log(`Regenerated spritesheet for visual ${i + 1}/${allVisuals.length}`);
    }
  } catch (error) {
