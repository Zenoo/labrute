...
const OAuth = {
    ...
    token: async (req, res) => {
        ...
        try {
            ...
            const existingUser = await client.query('select * from users where id = $1', [etwinUser.id]);
            // If user does not exist, create it
            if (((_a = existingUser.rows) === null || _a === void 0 ? void 0 : _a.length) === 0) {
                await client.query('INSERT INTO users(id, token, name) VALUES($1, $2, $3) RETURNING *', [etwinUser.id, token.accessToken, etwinUser.displayName.current.value]);
            }
            else {
                // If user exists, update it
                await client.query('UPDATE users SET name = $1, token = $2 WHERE id = $3 RETURNING *', [etwinUser.displayName.current.value, token.accessToken, etwinUser.id]);
            }
            // Fetch user data
            const userQuery = await client.query('select * from users where id = $1 and token = $2::text', [etwinUser.id, token.accessToken]);
